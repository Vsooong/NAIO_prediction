...
class Forecaster(nn.Module):
    ...
    def forward_by_stage(self, input, state, subnet, rnn):
        input, state_stage = rnn(input, state, seq_len=cfg.GLOBAL.OUT_LEN)
    ...
    def forward(self, hidden_states):
        ...
        input = self.forward_by_stage(None, hidden_states[-1], getattr(self, 'stage{len}'.format(len=self.blocks)),
                                      getattr(self, 'rnn{len}'.format(len=self.blocks)))
